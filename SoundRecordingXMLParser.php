...
class SonyXmlParser extends mysqli{
	...
	public function ListXmlFilesRecursively(){
		...
		foreach($RegexIterator as $name => $object){
			...
			if(copy($name, $newfile)){
				$insert = "INSERT INTO sony_xmllist(Filename,XML_status,Process_date) VALUE('".$info['basename']."',1,now())";
				$this->query($insert);
				unlink($name);
			}
			//unlink($name);
	...
	public function ProcessSonyRawXML(){
		...
		//$query = "SELECT srno,Filename,Process_date  FROM sony_xmllist where XML_status"; //-- For Testing
					...
					if(!empty($xml2assoc['xml_data'][0]['val'])){
						foreach($xml2assoc['xml_data'][0]['val'] as  $value){
							...
							switch($value['name']){
								...
								$this->InsertResourceList($ResourceListData,$xml2assoc['srno']);
								break;
								case "ReleaseList":
	...
	public function InsertDealList($DealListData,$MessageHeaderData,$ResourceListData,$ReleaseListData,$srno){
		...
		//		print_r($sonyDeal);
		/*foreach($ResourceListData as $SoundRecording){
		  if($SoundRecording['name'] == "SoundRecording"){
		  foreach($SoundRecording['val'] as $val){
		  }
		  }
		  }*/
...
public function InsertResourceList($ResourceListData,$srno){
	//return true;
	//print_r($ResourceListData);exit;
	$SoundRecordingType = null;
	$ISRC = null;
	...
	foreach($ResourceListData as $SoundRecording){
		if($SoundRecording['name'] == "SoundRecording"){
			foreach($SoundRecording['val'] as $val){
				switch($val['name']){
					case 'SoundRecordingType':
						$SoundRecordingType = $this->real_escape_string($val['val']); 
						break;
					...
					case 'SoundRecordingDetailsByTerritory':
						foreach($val['val'] as $SoundRecordingDetails){

							switch($SoundRecordingDetails['name']){
								case "Title":
									$Title = !empty($SoundRecordingDetails['val'][0]['val'])? $this->real_escape_string($SoundRecordingDetails['val'][0]['val']):null;
								break;
								case "DisplayArtist":
									foreach($SoundRecordingDetails['val'] as $arrDA){
										if($arrDA['name'] == 'PartyName'){
											$DisplayArtist[$SoundRecordingDetails['atr']['SequenceNumber']]['PartyName'] = $arrDA['val'][0]['val'];
										}
										if($arrDA['name'] == 'ArtistRole'){
											$DisplayArtist[$SoundRecordingDetails['atr']['SequenceNumber']]['ArtistRole'][] = $arrDA['val'];
										}
									}
								/*foreach($SoundRecordingDetails['val'] as $arrDA){
								  if($arrDA['name'] == "PartyName"){
								...
								/*if(!empty($SoundRecordingDetails['val'][0]['val'][0]['val'])){
								  $DisplayArtist[] = $SoundRecordingDetails['val'][0]['val'][0]['val'];
								  }*/

								break;
								case "ResourceContributor":
									foreach($SoundRecordingDetails['val'] as $arrRC){
										if($arrRC['name'] == 'PartyName'){
											$ResourceContributor[$SoundRecordingDetails['atr']['SequenceNumber']]['PartyName'] = $arrRC['val'][0]['val'];
										}
										if($arrRC['name'] == 'ResourceContributorRole'){
											$ResourceContributor[$SoundRecordingDetails['atr']['SequenceNumber']]['ResourceContributorRole'][] = !empty($arrRC['atr']['UserDefinedValue'])?$arrRC['atr']['UserDefinedValue']:$arrRC['val'];
										}
									}
								/*if(!empty($SoundRecordingDetails['val'][0]['val'][0]['val'])){
								  $UserDefinedValue = !empty($SoundRecordingDetails['val'][1]['atr']['UserDefinedValue'])? $this->real_escape_string($SoundRecordingDetails['val'][1]['atr']['UserDefinedValue']): $SoundRecordingDetails['val'][1]['val'];
								  $ResourceContributor[$UserDefinedValue][] = $SoundRecordingDetails['val'][0]['val'][0]['val'];
								  }*/
								break;
								case "LabelName":
									$Label = $this->real_escape_string($SoundRecordingDetails['val']);
								break;
								case "ParentalWarningType":
									$ParentalWarningType = $this->real_escape_string($SoundRecordingDetails['val']);
								break;
								case "Genre":
									$Genre = $this->real_escape_string($SoundRecordingDetails['val'][0]['val']);
								break;
								case "PLine":
									$Year = $this->real_escape_string($SoundRecordingDetails['val'][0]['val']);
								$plinetext = $this->real_escape_string($SoundRecordingDetails['val'][1]['val']);
								break;
								case "TechnicalSoundRecordingDetails":
									foreach($SoundRecordingDetails['val'] as $techDetails){
										$TechnicalSoundRecordingDetails[$ISRC][$techDetails['name']] = $techDetails['val'];	
									}
								$i++;
								break;
			...
			$NumberOfChannels = !empty($TechnicalSoundRecordingDetails[$ISRC]['NumberOfChannels'])? $this->real_escape_string($TechnicalSoundRecordingDetails[$ISRC]['NumberOfChannels']):null;
			$SamplingRate  = !empty($TechnicalSoundRecordingDetails[$ISRC]['SamplingRate'])? $this->real_escape_string($TechnicalSoundRecordingDetails[$ISRC]['SamplingRate']):null;
			$IsPreview  = !empty($TechnicalSoundRecordingDetails[$ISRC]['IsPreview'])? $this->real_escape_string($TechnicalSoundRecordingDetails[$ISRC]['IsPreview']):null;
			$FileURL  = !empty($TechnicalSoundRecordingDetails[$ISRC]['File'][0]['val'])? $this->real_escape_string($TechnicalSoundRecordingDetails[$ISRC]['File'][0]['val']):null;
			$HashSum  = !empty($TechnicalSoundRecordingDetails[$ISRC]['File'][1]['val'][0]['val'])? $this->real_escape_string($TechnicalSoundRecordingDetails[$ISRC]['File'][1]['val'][0]['val']):null;
			$AudioCodecType  =!empty($TechnicalSoundRecordingDetails[$ISRC]['AudioCodecType'])? $this->real_escape_string($TechnicalSoundRecordingDetails[$ISRC]['AudioCodecType']):null; 
		...
		if($SoundRecording['name'] == "Image"){
			foreach($SoundRecording['val'] as $img){
				switch($img["name"]){
					case "ImageType":
			...
			if(!empty($imgarr['File'][0]['val'])){
				$update ="UPDATE sony_songlist SET ImageURL = '".$imgarr['File'][0]['val']."',ImageHashSum='".$imgarr['File'][1]['val'][0]['val']."',ImageCodecType='".$imgarr['ImageCodecType']."',ImageHeight='".$imgarr['ImageHeight']."',ImageWidth='".$imgarr['ImageWidth']."',ImageResolution='".$imgarr['ImageResolution']."' where xmlid='".$xmlid."'";
				if (!$this->query($update)){
					printf("Errormessage: %s\n", $mysqli->error);
					$data = array('query'=>$update,'error'=>$mysqli->error,"line_number"=>__LINE__);
					file_put_contents('sony_xmllist.log',print_r($data,true),FILE_APPEND);
